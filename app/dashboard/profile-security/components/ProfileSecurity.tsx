...
const ProfileSecurity = () => {
	...
	const updateProfile = async () => {
		if (fName.length < 3)
			return toast.error("Full name is at least 2 character long");
		try {
			const data = {
				fullName: fName,
			};
			...
			let response;
			response = await axios.post(`${API_URL}profiles`, data, {
				headers: {
					"Content-Type": "application/json",
					Authorization: `Bearer ${token}`,
				},
	...
	return (
		<div className="max-lg:w-[80%] max-sm:w-[90%] w-[60%] flex flex-col gap-2 justify-start items-start p-5">
			<span className="text-base font-medium max-sm:text-sm text-zinc-600">
				{user?.email}
			</span>
			...
			<div className=" w-full border-t my-5 py-5 border-zinc-400 flex flex-col justify-start items-start gap-2 ">
				...
				<div className="rounded-[6px] upload-input flex justify-between items-center w-full">
					<input
						...
						className="bg-transparent  max-sm:text-sm text-[16px] font-normal p-3 outline-none h-full w-[96%]  placeholder:text-zinc-500  text-stone-800"
					/>
				</div>

				<PulsatingButton
					className="max-sm:text-base text-lg font-medium py-3 px-5 my-1 rounded-full flex justify-center items-center delay-5ms"
					disable={!fieldUpdated}
			...
			<div className=" w-full border-t my-5 py-5 border-zinc-400 flex flex-col justify-start items-start gap-2 ">
				...
				<span className="text-base font-medium text-zinc-600 max-sm:text-sm">
					No worries — we've got you covered! Click the button below, so you can
					update your password in no time.
				</span>
				<PulsatingButton
					onClick={changePass}
					className="max-sm:text-base text-lg font-medium py-3 px-5 my-1 rounded-full flex justify-center items-center"
				>
					Change password
				</PulsatingButton>
			</div>
