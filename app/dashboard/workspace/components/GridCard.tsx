    ...
    <div className="relative w-[300px]  list-card cursor-pointer hover:bg-[#f5f5f57e] bg-[#f5f5f52d] flex flex-col gap-1 justify-between items-start rounded-[8px]">
      ...
      <div className='flex w-full justify-between items-end p-[3px]'>
        ...
        <div className='flex justify-start items-center gap-2 p-2'>
          <span className=' text-sm font-normal text-zinc-700'>{status} {data.updatedAt.split('T')[0]}</span>
         {data.isExpired &&(<> <span className=' text-xs font-normal text-zinc-700'>|</span>
      ...
      {menu && <div ref={menuRef} className='absolute top-14 right-8 z-10  bg-[#ffffff] w-[115px] rounded-[8px] p-1 '>
        ...
        <span onClick={deleteFile} className=' hover:bg-[#a1a1a14d] p-1 rounded-[8px] w-full flex  justify-start items-center'>
          Delete
        </span>
        <span  onClick={()=>copyUrl(data.url)} className=' hover:bg-[#a1a1a14d] p-1 rounded-[8px] w-full flex  justify-start items-center'>
          Copy_link
        </span>
      </div>}
