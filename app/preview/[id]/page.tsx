      ...
      <div className=" w-full h-screen auth-bg flex items-end">
        <div className=" w-full  h-[87vh] flex  justify-center pt-11  gap-10 overflow-style">
          <div className='  w-[80%] h-[80vh] flex flex-col gap-2 justify-start items-start p-5'>
            {title ? <span className='text-stone-800 text-3xl font-semibold' >{title}</span> : <span className='text-stone-800 text-3xl font-semibold' >Preview files</span>}
            {description && <span className='text-lg font-normal text-zinc-600'>{description}</span>}
            ...
            <div className=' p-2 w-full  flex flex-wrap justify-start items-start gap-2'>
              {files.length !== 0 ? filteredFiles.map((val, i) => <>
                <div className="hover:bg-[#f5f5f57e] bg-[#f5f5f52d] w-full list-card cursor-pointer flex flex-col  justify-center items-center rounded-[8px] p-3">
                  ...
                  <div className='flex w-full justify-between items-end'>
                    <div className='flex justify-start items-center gap-3'>
                      <span className=' text-sm font-normal text-zinc-700'>{val.updatedAt.split('T')[0]
                      }</span>
                      {val.isExpired && (<> <span className=' text-xs font-normal text-zinc-700'>|</span>
                        <span className=' text-sm font-normal text-zinc-700'>Expired</span></>)}
                    </div>
                    <div className='flex justify-center items-center '>
                      <a download={true} href={val.url} className='list-btn-title-cont delay-5ms hover:bg-[#32323218] text-stone-800 p-2 rounded-full flex justify-center items-center'>
                        <LuDownload className='size-5' />
                        <span className='list-btn-title'>Download</span>
                      </a>
                      <span onClick={() => copyUrl(val.url)} className='list-btn-title-cont  delay-5ms hover:bg-[#32323218] text-stone-800 p-2 rounded-full flex justify-center items-center'>
                ...
                (<div className='text-stone-800 text-xl font-normal  w-full h-28 flex flex-col gap-3 justify-center items-center'>
                  <span className='text-center'>All the files recieved will appear here</span>
                </div>)}
            </div>
